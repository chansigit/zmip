...
def _table(rows, cols):
    ...
    body = "".join(
        "<tr>" + "".join(f"<td>{html.escape('' if r.get(c) is None else str(r.get(c)))}</td>" for c in cols) + "</tr>"
        for r in rows
    )
...
def _section_plan(outdir, plan):
    ...
    if os.path.exists(fig):
        parts.append('<div class="trio">' + img(fig) + "</div>")
    parts += [
        "<h3>Plan</h3>",
        ...
        _table(
            ...
            ("lineage", "coarse_labels", "n_cells", "zoom", "reason"),
        ),
        "</div>",
    ]
    if plan.get("notes"):
    ...
    for name, label in (
        ("lineage_counts.csv", "Cells per coarse label"),
        ...
        if os.path.exists(p):
            parts += [f"<details><summary>{label}</summary>{csv_table(p)}</details>"]
    return "".join(parts)
...
def _section_lineages(outdir, plan):
    ...
    for ln in zoomed:
        ...
        figs = [
            os.path.join(d, "figures", n)
            for n in ("annotation_umap_coarse.png", "annotation_umap_fine.png", "annotation_umap_removed.png")
        ]
        ...
        rows = [
            {
                "cluster": e["cluster_id"],
                "coarse": e["coarse_label"],
                "fine": e["fine_label"],
                "action": e["action"]
                + (f" → {e.get('reassign_to')}" if e["action"] == "reassign" else "")
                + (f" ({e.get('remove_reason')})" if e["action"] == "remove" else ""),
                "merge_target": e.get("merge_target") or "",
        ...
        parts.append(
            '<details><summary>per-cluster decisions</summary><div id="annotation-tables">'
...
def _section_final(outdir, kept_counts):
    ...
    if kept_counts:
        parts += [
            "<h3>Fine-label legend</h3>",
            '<p class="hint">Numbers on the fine UMAP above; ordered by coarse label, then size.</p>',
            '<div id="annotation-tables">',
            _table(kept_counts, ("id", "zmip_ann_coarse", "zmip_ann_fine", "n_cells")),
            "</div>",
        ]
    return "".join(parts)
...
def _section_removed(outdir):
    ...
    def count(rows, keys):
        ...
        for r in rows:
            ...
            out[k] = out.get(k, 0) + 1
        return [dict(zip(keys, k, strict=False), n_cells=v) for k, v in sorted(out.items(), key=lambda kv: -kv[1])]
    ...
    if rm:
        src = count(
            [
                {
                    "lineage": r["lineage"],
                    "source": ("agent: " + (r.get("remove_reason") or ""))
                    if r.get("annotate_remove") == "True"
                    else "subset preannotation filtering",
                }
                for r in rm
            ],
            ("lineage", "source"),
    ...
    if ra:
        parts += [
            f"<h3>Reassigned ({len(ra)} cells)</h3>",
            _table(
                count(ra, ("lineage", "reassign_to", "fine_label")), ("lineage", "reassign_to", "fine_label", "n_cells")
            ),
        ]
    return "".join(parts)
...
def _number(sections):
    present = [(a, label) for a, label in _LABELS.items() if any(f'<h2 id="{a}">{label}</h2>' in s for s in sections)]
    numbered = {a: f"{i}. {label}" for i, (a, label) in enumerate(present, 1)}
    out, toc = [], []
    for s in sections:
        anchor = next((a for a, label in present if f'<h2 id="{a}">{label}</h2>' in s), None)
        if anchor:
            s = s.replace(f'<h2 id="{anchor}">{_LABELS[anchor]}</h2>', f'<h2 id="{anchor}">{numbered[anchor]}</h2>', 1)
            subs, n = [], [0]
            ...
            if subs:
                toc.append(
                    '<div class="toc-sub">'
                    + "".join(f'<a href="#{sid}">{html.escape(re.sub("<.*?>", "", t))}</a>' for sid, t in subs)
                    + "</div>"
                )
        out.append(s)
